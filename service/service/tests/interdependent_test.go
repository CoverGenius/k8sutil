...
func TestInterdependentOneService(t *testing.T) {
	...
	contextWithTwoServices := service.AttachMetaData(validUnitYaml, "FAKE.yaml")
	...
	r := CreateTestMap(service.InterdependentRules(contextWithTwoServices))[lint.INTERDEPENDENT_AT_MOST_1_SERVICE]
	if r.Condition() {
	...
	contextWithNoServices := service.AttachMetaData(validUnitYaml, "FAKE.yaml")
	r = CreateTestMap(service.InterdependentRules(contextWithNoServices))[lint.INTERDEPENDENT_AT_MOST_1_SERVICE]
...
func TestInterdependentNamespaceRequired(t *testing.T) {
	...
	r := CreateTestMap(service.InterdependentRules(c))[lint.INTERDEPENDENT_NAMESPACE_REQUIRED]
	if r.Condition() {
...
func TestInterdependentNetworkPolicyForNamespace(t *testing.T) {
	...
	r := CreateTestMap(service.InterdependentRules(c))[lint.INTERDEPENDENT_NETWORK_POLICY_FOR_NAMESPACE]
	if r.Condition() {
...
func TestMatchingNamespace(t *testing.T) {
	...
	r := service.InterdependentRules(context) // need a list because the ID Is not unique
...
func SetGlobals() {
	...
	context = service.AttachMetaData(validUnitYaml, "FAKE.yaml")
	interdependentRules = CreateTestMap(service.InterdependentRules(context))
}
