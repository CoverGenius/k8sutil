...
func TestContainerExistsSecurityContext(t *testing.T) {
	...
	rule := CreateTestMap(service.ContainerRules(c, resource))[lint.CONTAINER_EXISTS_SECURITY_CONTEXT]
	if !rule.Condition() {
...
func TestContainerAllowPrivilegeEscalation(t *testing.T) {
	...
	d := resource.Resource.(*appsv1.Deployment)
	c := &d.Spec.Template.Spec.Containers[0]
	rule := CreateTestMap(service.ContainerRules(c, resource))[lint.CONTAINER_ALLOW_PRIVILEGE_ESCALATION_FALSE]
...
func TestContainerPrivileged(t *testing.T) {
	...
	d := resource.Resource.(*appsv1.Deployment)
	c := &d.Spec.Template.Spec.Containers[0]
	rule := CreateTestMap(service.ContainerRules(c, resource))[lint.CONTAINER_PRIVILEGED_FALSE]
	if !rule.Condition() {
...
func TestContainerResourceLimits(t *testing.T) {
	...
	d := resource.Resource.(*appsv1.Deployment)
	c := &d.Spec.Template.Spec.Containers[0]
	rule := CreateTestMap(service.ContainerRules(c, resource))[lint.CONTAINER_EXISTS_RESOURCE_LIMITS_AND_REQUESTS]
...
func TestContainerRequests(t *testing.T) {
	...
	d := resource.Resource.(*appsv1.Deployment)
	c := &d.Spec.Template.Spec.Containers[0]
	rule := CreateTestMap(service.ContainerRules(c, resource))[lint.CONTAINER_REQUESTS_CPU_REASONABLE]
	// original container should work
	if !rule.Condition() {
