...
func TestDeploymentProjectLabel(t *testing.T) {
	resource := service.AttachMetaData(deploymentYaml, "FAKEFILE.yaml")[0]
	rule := CreateTestMap(service.DeploymentRules(resource))[lint.DEPLOYMENT_EXISTS_PROJECT_LABEL]
...
func TestDeploymentContainerLiveness(t *testing.T) {
	...
	rule := CreateTestMap(service.DeploymentRules(resource))[lint.DEPLOYMENT_CONTAINER_EXISTS_LIVENESS]
...
func TestDeploymentContainerReadiness(t *testing.T) {
	...
	d := resource.Resource.(*appsv1.Deployment)
	c := &d.Spec.Template.Spec.Containers[0]
	rule := CreateTestMap(service.DeploymentRules(resource))[lint.DEPLOYMENT_CONTAINER_EXISTS_READINESS]
	// original container should work
	if !rule.Condition() {
...
func TestDeploymentAppK8sLabel(t *testing.T) {
	resource := service.AttachMetaData(deploymentYaml, "FAKEFILE.yaml")[0]
	rule := CreateTestMap(service.DeploymentRules(resource))[lint.DEPLOYMENT_EXISTS_APP_K8S_LABEL]
...
func TestDeploymentCorrectlySerialized(t *testing.T) {
	resources := service.AttachMetaData(deploymentYaml, "/Users/racheldowavic/FAKEFILE.yaml")
	if len(resources) != 1 {
...
func TestMain(m *testing.M) {
	...
	// get the rules
	rules = CreateTestMap(service.DeploymentRules(resources[0]))
	// tear-down
	os.Exit(m.Run())
}
